...
1. APIキーと認証の設定:
   - LINE、気象庁APIのAPIキーを取得し、/configsに設定ファイルとして追加。
   - 環境変数やシークレットマネージャーを活用してセキュアに管理。
2. APIクライアントの実装:
   - /pkg/lineapiにLINE Messaging APIとの連携ロジックを実装。
   - /pkg/weatherに気象庁APIとの連携ロジックを実装。
   - /pkg/cacheにRedisキャッシュのロジックを実装。
3. ユースケースの実装:
   - /internal/usecaseに天気通知と都市設定のビジネスロジックを実装。
4. インターフェースアダプタの実装:
   - /internal/interfaces/controllerにWebhookのハンドラを実装。
   - /internal/interfaces/repositoryにデータベースリポジトリを実装。
   - /internal/interfaces/presenterにLINEメッセージ送信ロジックを実装。
5. データベースのセットアップ:
   - PostgreSQLを設定し、必要なテーブルを作成。
   - マイグレーションツール（例：Flyway、Liquibase）を使用してスキーマ管理を自動化。
6. キャッシュの設定:
   - Redisをセットアップし、/pkg/cacheにキャッシュロジックを実装。
   - 天気情報のキャッシュを実装し、CronJobで定期的に更新。
7. スケジューリングシステムの実装:
   - 通知スケジュール管理のためにジョブ登録と実行ロジックを実装。
   - Kubernetes CronJobを活用して主要都市の天気情報を定期的に取得・キャッシュ。
8. テストの実装:
   - ユニットテストと統合テストを実装し、品質を確保。
   - テスト自動化をCI/CDパイプラインに組み込む。
9. デプロイとモニタリングの設定:
   - Dockerコンテナをビルドし、Fly.ioやKubernetesにデプロイ。
   - モニタリングツール（Prometheus、Grafanaなど）を設定し、システムの健全性を監視。
10. ユーザーインターフェースの設計:
    - LINEリッチメニューやメッセージテンプレートをデザインし、ユーザーフレンドリーなUIを構築。

まとめ

仕様変更に伴い、Geocoding APIとOpenWeatherMap APIの代わりに気象庁APIを使用して主要都市の天気情報を取得・管理する設計に変更しました。この設計により、ユーザーが個別の位置情報を入力する必要がなくなり、プライバシーリスクを軽減できます。以下のポイントを押さえながら、プロジェクトを進行してください：
1. 主要都市の選定と管理:
   - 事前に主要都市のリストを作成し、定期的に天気情報を取得してキャッシュします。
2. キャッシュ戦略の実装:
   - CronJobを使用して定期的に天気情報を取得し、RedisにキャッシュすることでAPI呼び出し回数を最適化します。
3. ユーザーインターフェースの調整:
   - ユーザーが主要都市を選択できるようにし、後からの変更も容易に行えるようにします。
4. データベース設計の最適化:
   - citiesテーブルを追加し、主要都市の情報を管理します。
5. テストとモニタリングの強化:
   - 各ユースケースとAPIクライアントのテストを充実させ、システムの健全性を監視します。
6. セキュリティ対策の徹底:
   - APIキーの管理やデータの暗号化を徹底し、セキュリティを確保します。